...
(function() {
  ...
  Collector = (function() {
    ...
    Collector.prototype._addGuide = function(file, data) {
      var id, item;
    ...
    Collector.prototype._addFeature = function(file, data) {
    ...
    Collector.prototype._addFile = function(file, data) {
      var namespace;
      namespace = '';
      file = file.replace(/(.*)\//, function(_, $1) {
        namespace = $1 + '/';
    ...
    Collector.prototype._classifyComments = function(comments) {
      ...
      return comments.forEach((function(_this) {
        return function(comment) {
          var i, id, j, last, len, ref, seperator, tag;
    ...
    Collector.prototype._getComments = function(type, path, file, data) {
    ...
    Collector.prototype._getType = function(file) {
      if (/\.coffee$/.test(file)) {
        return 'coffeescript';
    ...
    Collector.prototype.run = function() {
      var all_comments, comments, data, file, file_count_read, j, len, path, ref, ref1, type;
      all_comments = [];
      file_count_read = 0;
      ref = this.contents;
      for (j = 0, len = ref.length; j < len; j++) {
        ref1 = ref[j], path = ref1.path, file = ref1.file, data = ref1.data;
        type = this._getType(file);
        switch (type) {
          case 'guide':
            this._addGuide(file, data);
            break;
          case 'feature':
            this._addFeature(file, data);
            break;
          case 'coffeescript':
          case 'javascript':
          case 'page':
            comments = this._getComments(type, path, file, data);
            if (comments != null) {
