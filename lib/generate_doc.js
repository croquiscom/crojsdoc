...
(function() {
  ...
  copyResources = function(source, target, callback) {
    ...
    return exec("rm -rf " + target + "/* ; mkdir -p " + target + " ; cp -a " + source + "/bootstrap " + source + "/google-code-prettify " + source + "/tocify " + source + "/style.css " + target, function() {
      return callback();
    });
  };
  ...
  render = function(result, genopts, options, template, output) {
    ...
    options.cache = true;
    return jade.renderFile("" + genopts.template_dir + "/" + template + ".jade", options, function(error, result) {
      var output_file;
      if (error) {
  ...
  generate = function(paths, genopts) {
    var all_comments, content, e, external_types, file_count_read, output_dir, type, url;
    ...
    refineResult(result);
    return copyResources(__dirname, genopts.doc_dir, function() {
